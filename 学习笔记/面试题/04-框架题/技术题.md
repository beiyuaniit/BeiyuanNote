...
spring对循环依赖的处理有三种情况： 
①构造器的循环依赖：这种依赖spring是处理不了的，直 接抛出BeanCurrentlylnCreationException异常。 因为要通过构造器来创建初始对象
②非单例循环依赖：无法处理。  
...
- 步骤
  - A的某个field或者setter依赖了B的实例对象，同时B的某个field或者setter依赖了A的实例对象”这种循环依赖的情况。A首先完成了， 初始化的第一步（createBeanINstance实例化），并且将自己提前曝光到singletonFactories中。(底层缓存)     三级缓存
  - 发现自己依赖对象B，此时就尝试去get(B)，发现B还没有被create，所以走create流程，B在初始化第一步的时候发现自己依赖了对象A，于是尝试get(A)，尝试一级缓存singletonObjects(肯定没有，因为A还没初始化完全)，尝试二级缓存earlySingletonObjects（也没有）
  - 尝试三级缓存singletonFactories，由于A通过ObjectFactory将自己提前曝光了，所以B能够通过ObjectFactory.getObject拿到A对象(虽然A还没有初始化完全，但是总比没有好)，B拿到A对象后，**将A放入到2级缓存**顺利完成了初始化阶段1、2、3，完全初始化之后将自己放入到一级缓存singletonObjects中
  - 返回A中，A此时能拿到B的对象顺利完成自己的初始化阶段2、3，最终A也完成了初始化，进去了一级缓存singletonObjects中

- 为什么要3级，2级不行吗
  - 面向切面编程(AOP),如果查到第三级缓存，调用动态代理 对象的#getObject，因为每次调用都会拿到一个新的对象
  - 多个切面同时进行初始化，就有可能不是单例了 
...
- 静态代理 - 指使用 AOP 框架提供的命令进行编译，从而在编译阶段就可生成 AOP 代理类，因此也称为编译时增强；
  - 编译时编织（特殊编译器实现）
  - 类加载时编织（特殊的类加载器实现）。
  - 静态代理是一种设计模式，通过创建一个代理类来封装和控制对真实对象的访问。要实现静态代理，可以按照以下步骤进行操作：
    1. 定义接口：首先，需要定义一个接口，该接口将被代理类和代理类都实现，该接口将定义代理对象和真实对象共同的方法。
    2. 创建代理类：创建一个代理类，该代理类实现了定义的接口。代理类中应该包含一个对真实对象的引用。
    3. 实现代理方法：在代理类中，实现接口中的方法。通常，在代理方法中会调用真实对象相应的方法，同时可以添加一些额外的操作。
    4. 使用代理类：在需要使用真实对象的地方，使用代理类代替真实对象进行操作。通过代理类可以控制对真实对象的访问，并在必要时执行一些额外的操作。
...
- 事务的传播特性（Transaction Propagation）是指在一个分布式环境中，事务的执行如何在多个事务边界之间传播和影响的规则或策略。事务的传播特性可以控制事务的边界和范围，确保多个事务能够按照期望的方式进行协调和执行。

- 以下是常见的事务传播特性及其含义：

  1. REQUIRED：如果当前存在事务，则加入该事务；如果没有事务，则创建一个新事务。这是最常用的传播特性。比如在一个方法A中调用方法B，如果方法B已经在一个事务中执行，方法A将加入该事务继续执行，如果方法B没有在事务中执行，方法A将创建一个新的事务来执行方法B。
  2. REQUIRES_NEW：创建一个新事务，如果当前存在事务，则挂起当前事务。无论外部是否存在事务，该方法都会创建一个新的事务来执行，外部事务将被挂起。
  3. SUPPORTS：如果当前存在事务，则加入该事务；如果没有事务，则以非事务方式执行。如果外部存在事务，方法将加入该事务执行，如果外部不存在事务，方法将以非事务方式执行。
  4. MANDATORY：如果当前存在事务，则加入该事务；如果没有事务，则抛出异常。要求该方法在一个已存在的事务中执行，如果外部不存在事务，则抛出异常。
  5. NOT_SUPPORTED：以非事务方式执行操作，如果当前存在事务，则挂起当前事务。无论外部是否存在事务，该方法都会以非事务方式执行。
  6. NEVER：以非事务方式执行操作，如果当前存在事务，则抛出异常。要求方法在没有事务的上下文中执行，如果外部存在事务，则抛出异常。
  7. NESTED：如果当前存在事务，则创建一个嵌套事务，并在新的保存点处设置一个保存点。嵌套事务可以独立于外部事务进行提交或回滚。如果外部存在事务，该方法将在外部事务的上下文中创建一个嵌套事务，可以使用保存点来控制嵌套事务的提交或回滚。

  选择适当的事务传播特性可以根据不同的需求和业务场景来灵活地管理事务的边界和范围，确保事务的执行和协调符合预期。
...
- 使用MQ的场景很多，主要有三个：解耦、异步、削峰。
  - 解耦：假设现在，日志不光要插入到数据库里，还要在硬盘中增加文件类型的日志，同时，一些关键日志还要通过邮件的方式发送给指定的人。那么，如果按照原来的逻辑，A可能就需要在原来的代码上做扩展，除了B服务，还要加上日志文件的存储和日志邮件的发送。但是，如果你使用了MQ，那么，A服务是不需要做更改的，它还是将消息放到MQ中即可，其它的服务，无论是原来的B服务还是新增的日志文件存储服务或日志邮件发送服务，都直接从MQ中获取消息并处理即可。这就是解耦，它的好处是提高系统灵活性，扩展性。一个消息发送给多个接受者，可灵活增加和减少接收者
