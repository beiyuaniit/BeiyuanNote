...
demo.controller.HelloController.java
...
取这个名会自动被spring-boot接管（放一些全局的配置
...
​	可以在该网页直接Ctrl+F搜索（好像所有网页都可以这样搜索
...
4. 见到的  *-spring-boot-starter： 第三方为我们提供的简化开发的场景启动器。如beiyuan-spring-boot-starter
...
- @ComponentScan
- 它们是Spring的基本标签，在Spring Boot中并未改变它们原来的功能。都能直接用
...
**条件装配：根据Conditional指定的条件，决定标注的类或方法的是否进行组件注入**
...
2. Registrar类注册指定的一个包下的所有组件。（MainApplication所在包
...
4. SpringFactoriesLoader类从`META-INF/spring.factories`位置来加载一个文件。
...
虽然我们127个场景的所有自动配置启动的时候默认全部加载，但是`xxxxAutoConfiguration的类或方法上有@Conditional，按照条件装配规则
...
```java
//给容器加入文件上传解析器。总的功能，把容器中不符合规范的文件解析器重命名为 multipartResolver（必须是这个
@Bean
@ConditionalOnBean(MultipartResolver.class)  //容器中有这个类型组件
@ConditionalOnMissingBean(name = DispatcherServlet.MULTIPART_RESOLVER_BEAN_NAME) //容器中没有这个名字 multipartResolver 的组件
public MultipartResolver multipartResolver(MultipartResolver resolver) {
	//给@Bean标注的方法传入了对象参数，这个参数的值就会从容器中找。注入
	//SpringMVC multipartResolver。防止有些用户配置的文件上传解析器不符合规范
	// Detect if the user has created a MultipartResolver but named it incorrectly
	return resolver;
}

//matchIfMissing = true,缺失了也可以？
...
- 查看自动配置了哪些（选做）
	...
	- application.properties配置文件中加上debug=true开启自动配置报告。控制台输出报告
	  - Negative（不生效）也说了哪些条件不匹配
	  - Positive（生效）
- 是否需要修改
	- 参照文档修改配置项
...
在IDEA中，菜单栏New -> Project -> Spring Initailizr->定制化配置场景启动器等。
...
访问 ： 当前项目根路径/ + 静态资源名    （浏览器也可以直接访问。。

localhost:8080/aixin.png
...
目录没改变，就是中间要加上个前缀才能访问

项目路径+static-path-pattern + 静态资源名 = 静态资源文件夹下找

​	即使改变了默认的静态资源路径，也是上面这个路径。

​	localhost:8080/res/aixin.png

### webjar

可用jar方式添加css，js等资源文件，

官网：[https://www.webjars.org/](https://www.webjars.org/)   可以找这样的依赖
...
- 给容器中配置的内容：
  - 配置文件的相关属性的绑定：WebMvcProperties和spring.mvc绑定、ResourceProperties和spring.resources绑定
...
spring:
  resources:
    add-mappings: false   #禁用所有静态资源规则。不能通过前端直接访问
```
...
WelcomePageHandlerMapping(TemplateAvailabilityProviders templateAvailabilityProviders,
    ...
    if (welcomePage != null && "/**".equals(staticPathPattern)) {
        //要用欢迎页功能，必须是/**，不能手动配置其他，已经写死了，否则访问不到
        logger.info("Adding welcome page: " + welcomePage);
        setRootViewName("forward:index.html");
    }
    else if (welcomeTemplateExists(templateAvailabilityProviders, applicationContext)) {
        //若不存在静态的欢迎页，调用Controller /index
...
spring:
  mvc:
    hiddenmethod:
      filter:
        enabled: true   #开启页面表单的Rest功能，因为有时是直接请求数据，页面给别人写，所以是选择性开启
```
...
- Rest原理（表单提交要使用REST的时候）
  ...
  - **请求过来被**`HiddenHttpMethodFilter`拦截
    - 请求是否正常，并且是POST
      ...
      - **原生request（post），包装模式requestToUse = new HttpMethodRequestWrapper(request, method);改变了请求方式，重写了getMethod方法，返回的是传入的值。**
      - **过滤器链放行的时候用wrapper。以后的方法调用getMethod是调用requestWrapper类的。**
...
public class HiddenHttpMethodFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		if ("POST".equals(request.getMethod()) && request.getAttribute(WebUtils.ERROR_EXCEPTION_ATTRIBUTE) == null) {//所以delete和put请求首先得是post请求
            //this.methodParam的值是_method，通过参数名获取参数值
			String paramValue = request.getParameter(this.methodParam);
			if (StringUtils.hasLength(paramValue)) {
				String method = paramValue.toUpperCase(Locale.ENGLISH);//变为大写
...
- Rest使用客户端工具。
  - 如PostMan可直接发送put、delete等方式请求。不是表单发送
  - 则不会包装成requestWrapper
...
public class WebConfig{
    ...
    public HiddenHttpMethodFilter hiddenHttpMethodFilter(){
        ...
        methodFilter.setMethodParam("_m");//把_method改为_m
        return methodFilter;
    }    
}
```
...
其中，RequestMappingHandlerMapping保存了所有`@RequestMapping` 和`handler`的映射规则。（手写那些在这里
...
4. 如cookie被禁用了，无法获取jsessionid，可以通过矩阵变量来传递（url重写

    /abc;jsessionid=xxx             当然用普通方式也可以传递，用这种是为了区分
...
public class WebConfig implements WebMvcConfigurer {
    ...
    public void configurePathMatch(PathMatchConfigurer configurer) {
        ...
        // 不移除；后面的内容。矩阵变量功能就可以生效。semicolon:分号
...
public class ParameterTestController {
    ...
    @GetMapping("/cars/{path}")//因为矩阵变量绑定在路径中，从路径中取，不能写成/cars/sell，必须要有路径变量才能解析
    ...
    // /boss/1;age=20/2;age=10      //从一层路径中取，所以每层都可以绑定矩阵变量
    //同名的多个矩阵变量
...
public class ServletInvocableHandlerMethod extends InvocableHandlerMethod {
	...
	protected Object[] getMethodArgumentValues(NativeWebRequest request, @Nullable ModelAndViewContainer mavContainer,
		...
		for (int i = 0; i < parameters.length; i++) {
            ...
            //查看resolvers是否有支持，遍历26个is.resolvers.supportsParameter(parameter)) {
...
封装过程用到`ServletModelAttributeMethodProcessor`解析
...
public class ServletModelAttributeMethodProcessor extends ModelAttributeMethodProcessor {
	...
	public final Object resolveArgument(MethodParameter parameter, @Nullable ModelAndViewContainer mavContainer,
		...
		if (mavContainer.containsAttribute(name)) {
			...
			// Create attribute instance，一个空的pojo对象
			try {
		...
		if (bindingResult == null) {
			...
			if (binder.getTarget() != null) {
				if (!mavContainer.isBindingDisabled(name)) {
                    //web数据绑定器，将请求参数的值绑定到指定的JavaBean里面**，底层利用反射进行赋值
...
[32、请求处理-【源码分析】-各种类型参数解析原理 - 返回值处理器](#)有讨论**ReturnValueHandler**。返回值解析器。现在直接看看重点：
...
3. `RequestResponseBodyMethodProcessor` 可以处理返回值标了`@ResponseBody` 注解的。
	- 利用 `MessageConverters` 进行处理 将数据写为json
		1. 内容协商（浏览器默认会以请求头中的Accept属性告诉服务器他能接受什么样的内容类型）
...
//                parameterStrategy.setParameterName("ff");
                ...
                //还需添加请求头处理策略，否则请求头accept:application/json、application/xml则会失效。找不到适配的，认为都能适配，则选择第一个，无论发送application/xxx都返回json
...
springboot打包方式是jar，是个压缩包，jsp不支持在压缩包内编译，所以springboot默认不支持jsp

可引入第三方模板引擎技术来渲染

thymeleaf面向的是后端，不是高性能的，所以高并发还是前后端分离
